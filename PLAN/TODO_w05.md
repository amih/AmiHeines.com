# Week 5 — Frontend wiring + Video #4

**Dates:** 2026-06-01 to 2026-06-07
**Phase:** 1 — Cohort
**Target hours:** ≤20 (hard cap)

---

## Rainmaker (3h)

### Pricing anchor research (3h)
- [ ] Document avg Web2→Web3 dev salary delta (LinkedIn Salary, Glassdoor, public reports). Save anchor numbers to `PLAN/cohort_offer.md` evidence section.
- [ ] Compare to: 4-yr CS degree cost, Lambda School clone cost, ConsenSys academy cost. Build "alternatives" anchor table.

---

## Build (10h)

### Frontend talking to chain (10h)
- [ ] React app: connect wallet (anchor / scatter or Vaulta-native), call contract actions, render game state.
- [ ] Tailwind UI, mobile-friendly.
- [ ] Push, deploy to Vercel/Netlify subdomain `game.amiheines.com` (or similar).

---

## Content (5h)

### Video #4 — "Building a Web3 Frontend with Claude Code" (5h)
- [ ] Show wallet integration, contract call wiring, error-handling on camera.
- [ ] CTA: waitlist; tease that cohort enrollment opens in 5 weeks.

---

## EAD / admin (2h)

- [ ] Auto pipeline.
- [ ] Mid-cohort planning: outline 6-week curriculum modules, save to `PLAN/cohort_curriculum.md` v0.

---

## End-of-week checkpoints

- [ ] Game playable end-to-end on testnet.
- [ ] Video #4 live.
- [ ] Pricing anchor evidence documented.
- [ ] Curriculum v0 outlined.
- [ ] Hours ≤20.
