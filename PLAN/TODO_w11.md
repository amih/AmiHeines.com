# Week 11 — Onboard cohort + Video #9

**Dates:** 2026-07-13 to 2026-07-19
**Phase:** 1 — Cohort
**Target hours:** ≤20 (hard cap)

---

## Rainmaker (8h)

### Cohort onboarding (8h)
- [ ] Welcome email to all paid cohort students.
- [ ] Discord access + intro thread.
- [ ] Send Module 1 prep work + first call calendar invite.
- [ ] Final close window: extend offer to 5 strongest challenge attendees who hesitated. Hard close Wed EOD.

---

## Build (3h)

### Module 1 materials prep (3h)
- [ ] Lesson 1 slides + live-demo plan.
- [ ] Homework spec written.

---

## Content (5h)

### Video #9 — "I Just Sold N Spots in My $5k Cohort — Here's What Worked" (5h)
- [ ] Honest retro of challenge → cohort numbers.
- [ ] Frame as Garnon-relevant lesson: how Cochrane's paid-event model worked.
- [ ] CTA: waitlist for Cohort #2 (date TBD by W21).

---

## EAD / admin (4h)

- [ ] Auto pipeline.
- [ ] Reflection day: did everything fit ≤20h? Adjust W12+ if not.

---

## End-of-week checkpoints

- [ ] All paid cohort students in Discord.
- [ ] Module 1 prep complete.
- [ ] Video #9 live.
- [ ] Cohort #1 seat count finalized.
- [ ] Hours ≤22 (week-after-launch tail).
